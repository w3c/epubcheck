Feature: EPUB 3 — Accessibility
  ...
  Rule: allow properties from the EPUB accessibility vocabulary

    Example: Verify an 'a11y' prefix used in metadata properties without being declared
      When checking file 'property-prefix-a11y-not-declared-valid.opf'
      Then no errors or warnings are reported

    Example: Report unknown 'a11y' metadata
    ...
    Example: Verify an 'a11y:certifierCredential' property can be defined as a link
      When checking file 'link-rel-a11y-certifierCredential-valid.opf'
      Then no errors or warnings are reported

    Example: Allow using the 'a11y:exemption' property in package metadata
      When checking file 'metadata-a11y-exemption-valid.opf'
      Then no errors or warnings are reported

    Example: Allow using the 'a11y:contactEmail' property in package metadata
      When checking file 'metadata-a11y-contactEmail-valid.opf'
      Then no errors or warnings are reported
