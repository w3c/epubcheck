Feature: EPUB 3 Package Document
 ...
 Scenario: 'dc:identifier' starting with "urn:uuid:" should be a valid UUID  
  ...
  Scenario: 'dc:subject' can be refined with a code term and authority
  	When checking file 'metadata-subject-authority-valid.opf'
  	Then no errors or warnings are reported
  	
  Scenario: 'dc:subject' authority must be associated to a term
  	When checking file 'metadata-subject-authority-no-term-error.opf'
  	Then error RSC-005 is reported
  	And the message contains "A term property must be associated"
  	And no other errors or warnings are reported
  	
  Scenario: 'dc:subject' authority must not be defined more than once
  	When checking file 'metadata-subject-authority-multiple-error.opf'
  	Then error RSC-005 is reported
  	And the message contains "Only one pair of authority and term properties"
  	And no other errors or warnings are reported
  	
  Scenario: 'dc:subject' term must be associated to an authority
  	When checking file 'metadata-subject-term-no-authority-error.opf'
  	Then error RSC-005 is reported
  	And the message contains "An authority property must be associated"
  	And no other errors or warnings are reported
  	
  Scenario: 'dc:subject' term must not be defined more than once
  	When checking file 'metadata-subject-term-multiple-error.opf'
  	Then error RSC-005 is reported
  	And the message contains "Only one pair of authority and term properties"
  	And no other errors or warnings are reported
